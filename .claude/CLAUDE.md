...
- Lint and check for type errors after adjusting code according to the project's setup. Prioritize automatic fixed linting.
...
- If you need to run commands (lint, test, tsc, etc.) in a monorepo, use `npm run test --workspace <workspace-name-from-package-json>` to run commands in the correct package. Same goes for installing dependencies.
