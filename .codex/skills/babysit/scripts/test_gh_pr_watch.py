...
def test_pending_review_feedback_surfaces_only_after_publication(monkeypatch):
    ...
    assert next(
        item for item in published_items if item["kind"] == "review_comment"
    )["thread_root_id"] == "19"


def test_unacknowledged_review_comment_remains_pending_across_polls(monkeypatch):
    state = {}
    review_comment = {
        "id": 20,
        "in_reply_to_id": None,
    ...
    first_poll = gh_pr_watch.fetch_new_review_items(
        sample_pr(),
        state,
        fresh_state=True,
    )
    second_poll = gh_pr_watch.fetch_new_review_items(
        sample_pr(),
        state,
        fresh_state=False,
    )

    assert [(item["kind"], item["id"]) for item in first_poll] == [
        ("review_comment", "20")
    ]
    assert [(item["kind"], item["id"]) for item in second_poll] == [
        ("review_comment", "20")
    ]


def test_acknowledged_review_comment_leaves_pending_queue(monkeypatch):
    state = {}
    review_comment = {
        "id": 20,
        "in_reply_to_id": None,
    ...
    assert state["pending_review_comment_ids"] == []
    assert state["acknowledged_review_comment_ids"] == ["20"]


def test_legacy_seen_review_comment_can_be_requeued(monkeypatch):
    state = {"seen_review_comment_ids": ["20"]}
    review_comment = {
        "id": 20,
        "in_reply_to_id": None,
        "pull_request_review_id": 10,
    ...
    gh_pr_watch.requeue_review_items(
        state,
        [("review_comment", "20")],
    )

    pending_items = gh_pr_watch.fetch_new_review_items(
        sample_pr(),
        state,
        fresh_state=False,
    )

    assert [(item["kind"], item["id"]) for item in pending_items] == [
        ("review_comment", "20")
    ]
